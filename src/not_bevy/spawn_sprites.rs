...
use super::{
    constants_and_startup::{
        LEVEL1_HEIGHT, MAX_SIDE_DIST, PLAYER_SIZE, above_player_spawn_dist,
        below_player_spawn_dist, side_player_spawn_dist,
    },
    player_components::*,
};
...
impl Plugin for FurnPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(FixedUpdate, (despawn_furns, animate_furns, spawn_furns));
    }
}
...
fn spawn_furns(
    ...
    if rng.random_bool(0.033) {
        ...
        for furn_transform in furns {
            if furn_transform.translation.x > min_x
                && furn_transform.translation.x < max_x
                && furn_transform.translation.y > min_y
                && furn_transform.translation.y < max_y
            {
                furns_in_box += 1;
            }
        }
        ...
        while is_incompatible {
            (x_size, y_size, x_location, y_location) = (
            ...
            if ((furn_x1 >= player.translation.x - 16.0)
                && (furn_x2 <= player.translation.x + 16.0))
                && ((furn_y1 >= player.translation.y - 16.0)
                    && (furn_y2 <= player.translation.y + 16.0))
            {
                // still incompatible
